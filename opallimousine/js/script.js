(function($) {
	...
	//Select Car Tabs Box
	if($('.select-car-tabs').length){
		$('.select-car-tabs .tab-buttons .tab-btn').on('click', function(e) {
			e.preventDefault();
			var target = $($(this).attr('data-tab'));
			...
			if ($(target).hasClass('active-tab')){
				...
				target.parents('.select-car-tabs').find('.tab-buttons').find('.tab-btn').removeClass('active-btn');
				$(this).addClass('active-btn');
				target.parents('.select-car-tabs').find('.tabs-content').find('.tab').removeClass('active-tab');
				$(target).addClass('active-tab');
			}
		});
	}
...
// https://codepen.io/sazhin/pen/bwdGBO
